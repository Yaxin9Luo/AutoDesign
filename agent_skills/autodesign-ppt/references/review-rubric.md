...
- a `w/o`/`full` comparison authorized by architecture counts rather than an
  observed outcome;
- a token-bound structure value such as `2 heads`, `2-stage`, or `depth: 4`
  accepted as an outcome because a metric word appears elsewhere, or a real
  metric-bound value rejected merely because a distant structure noun exists;
