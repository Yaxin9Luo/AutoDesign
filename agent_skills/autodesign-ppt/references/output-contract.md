...
ground an experimental slide. Localized absence grammar such as
`本文缺乏鲁棒性评估` or `本文无定性分析` is also rejected without treating
method modifiers such as `无监督` as absence. Evidence is evaluated clause by
clause. One complete, non-negated positive clause may ground the role even when
...
Token-attached values such as `2 heads`, `2-stage`, `depth is exactly 4`,
`head-count of 4`, or `head count has a value of 4` remain structure counts; a
metric word elsewhere cannot relabel them as outcomes. A distant structure
phrase cannot steal a separately metric-bound value, such as the `79.4%` in
`accuracy for the 2-stage configuration is 79.4%`, or in
`accuracy is 79.4% across two stages`. When neither
